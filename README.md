...
HarmonyOS 项目在任务的第一个 Hook 通过任务锁执行一次只读环境探测，后续 Stop 和设备阶梯只
读取 `.runtime-correction/tasks/<taskId>/environment/harmonyos.json`，不重复调用工具。Windows
...
快照明确区分“工具已安装”“可启动模拟器”和“已有连接 target”。例如 Emulator 已安装但
...
对 HARD UI 验收，单张截图或 install/launch smoke 不能证明业务通过。缺少当前的
build → install → launch → UI action → assertion/capture 链时，M12/M13/M15 对象保持
`UNVERIFIED`；未执行不伪装成测试失败。环境知识和华为官方文档链接维护在
