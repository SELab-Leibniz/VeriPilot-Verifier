...
import { decisionScope } from "./runtime-v2/decision-scope.mjs";
...
export async function persistResult({
  ...
  if (scope) {
    // Legacy latest reports must not publish a review which later loses its
    // cancellation/version race. Persist the diagnostic projection at commit.
    (scope.outputs ??= []).push(structuredClone({ result, output, cwd, triggerFile, stageSpecification, outputKey }));
    const names = ["diagnostic.md", "result.json", ...(result.status === "failed" && stageSpecification ? ["spec.md"] : []), "patch.diff"];
    const roundFiles = names.map(name => path.join(roundDirectory, name));
    const latestFiles = names.map(name => path.join(latestDirectory, name));
