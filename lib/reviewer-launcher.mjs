...
function frozenPlan(executable, argsPrefix = []) {
  return Object.freeze({ executable, argsPrefix: Object.freeze([...argsPrefix]) });
}
...
export function normalizeReviewerRuntime(value, { projectRoot, platform = process.platform } = {}) {
  ...
  if (Object.keys(value).some((key) => key !== "executable" && key !== "argsPrefix")) {
...
export async function resolveReviewerLaunchPlan({
  ...
  if (override !== undefined) {
    ...
    // Select the entire launch pair; a program override never inherits a wrapper prefix.
...
export function buildReviewerInvocation(plan, cliArgs) {
  return { executable: plan.executable, args: [...plan.argsPrefix, ...cliArgs] };
}
