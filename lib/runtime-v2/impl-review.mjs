...
//
// Ownership partition: only source-targeted M09/M12 obligations, as selected
// by review-routing.mjs. M11 and document/process M12 objects stay outside
// this reviewer's evidence boundary. Its judgements replace the general ones for
...
import { implementationPopulation } from "./review-routing.mjs";

const IMPL_METRIC_PREFIXES = ["M09:", "M12:"];
...
export function failedImplementationReview(population, error) {
  return {
    status: "FAILED",
    summary: "Implementation verification could not complete.",
    findings: [],
    metricObjectJudgements: Object.values(population.metrics).flat().map((object) => ({
      objectId: object.objectId,
      judgement: "CHECKER_ERROR",
      reason: `Required implementation review failed: ${error.message}`,
      evidence: [],
    })),
    sourceManifestCount: null,
    deviceAssurance: null,
 ...
 * never require the LLM reviewer to succeed). Failure is explicit and owned
 * objects become CHECKER_ERROR; the caller uses bounded infrastructure retries.
 */
export async function runImplementationReview({
  ...
  const routing = implementationPopulation(population, { groundTruth, stage });
  if (routing.skipReason) return {
    status: "SKIPPED", skipReason: routing.skipReason,
    summary: `Implementation review skipped: ${routing.skipReason}.`,
    findings: [], metricObjectJudgements: [], sourceManifestCount: 0,
    deviceAssurance: null, reviewerError: null,
  };
  const ownedObjects = Object.values(routing.population.metrics).flat();
  const ownedIds = new Set(ownedObjects.map((object) => object.objectId));
  ...
  if (ownedObjects.length === 0) return {
    status: kitFindings.length ? "COMPLETED" : "SKIPPED",
    skipReason: kitFindings.length ? null : "NO_IMPLEMENTATION_OBJECTS",
    summary: "No source-owned metric objects; deterministic capability checks only.",
    findings: kitFindings, metricObjectJudgements: [], sourceManifestCount: sourceManifest.length,
    deviceAssurance: null, reviewerError: null,
  };
  ...
  const request = {
    ...
    instructions: [
      ...
      "Scope: judge exactly the supplied implementation-owned M09/M12 objects. Document content and process/tool-history objects are owned by other checks and are not supplied. Never invent or omit objects.",
  ...
  try {
    ...
    if ((result?.metricObjectJudgements ?? []).some((object) => !ownedIds.has(object.objectId))) {
      throw new Error("Implementation reviewer returned an object outside its assigned evidence ownership.");
    }
  } catch (error) {
    // Keep deterministic findings AND explicitly fail every owned object.
    // A reviewer fault must never fall back to the general reviewer's PASS.
    reviewerError = error;
  } finally {
    await handle?.close?.();
  }
  return {
    status: reviewerError ? "FAILED" : "COMPLETED",
    ...
    metricObjectJudgements: reviewerError
      ? failedImplementationReview(routing.population, reviewerError).metricObjectJudgements
      : [
        ...filterOwnedJudgements(result?.metricObjectJudgements, ownedIds),
        ...ownedObjects.filter((object) => !(result?.metricObjectJudgements ?? [])
          .some((judgement) => judgement.objectId === object.objectId)).map((object) => ({
          objectId: object.objectId, judgement: "CHECKER_ERROR",
          reason: "Implementation reviewer omitted this owned object.", evidence: [],
        })),
      ],
