...
/**
 * Default heading pattern locating kit-checklist sections in the material
 * markdown: content-based (headings mentioning kit/dependency/依赖/清单), with
 * the legacy "10.1" numbering kept as an alternative. Over-capture is safe —
 * table rows are still shape-filtered to catalog names.
 */
export const DEFAULT_CHECKLIST_SECTION_PATTERN = "^#{1,4}\\s*(?:10\\.1\\b|[^#]*(?:[Kk]it|[Dd]ependenc|依赖|清单))";
...
/**
 * Markers in a table header or section title saying the table lists CANDIDATE
 * or feasibility-graded capabilities rather than committed usage. Kits from
 * such a table are advisory (hedged), never blocking obligations — the
 * material itself declines to commit.
 */
const CANDIDACY_MARKERS = /候选|可行性|备选|feasib|candidate|option/iu;
/** Header cell naming the kit column (checked before the positional fallback). */
const KIT_HEADER_PATTERN = /kit|依赖|dependenc/iu;
/**
 * Cell-level markers saying this particular entry is a candidate, a future
 * option, or explicitly not committed. Such kits are advisory (hedged) even
 * inside a committed usage table.
 */
const CELL_HEDGE_MARKERS = /候选|备选|待定|待.{0,4}评审|未来|暂不|不在|poc|tbd|candidate|tentative/iu;
...
export function cellIsHedged(cell) {
  return CELL_HEDGE_MARKERS.test(String(cell ?? ""));
}
...
export function parseKitManifest(markdown, {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if ((sectionBoundary || !inSection) && headingPattern.test(line)) {
      ...
      candidacyTable = CANDIDACY_MARKERS.test(currentSection ?? "");
      continue;
    }
    if (sectionBoundary) {
      inSection = false;
    ...
    if (/^\|[\s|:-]*\|?$/u.test((lines[index + 1] ?? "").trim())) {
      const headerIndex = cells.findIndex((cell) => KIT_HEADER_PATTERN.test(cell));
      column = headerIndex >= 0 ? headerIndex : fallbackColumn;
      candidacyTable = CANDIDACY_MARKERS.test(currentSection ?? "")
        || (headerIndex >= 0 && CANDIDACY_MARKERS.test(cells[headerIndex]));
      continue;
    }
    if (cells.length < column + 2) continue;
    const hedged = candidacyTable || cellIsHedged(cells[column]);
...
export async function runImplementationReview({
  ...
  const device = await deviceVerifier({
    ...
    environmentSnapshot: harmonyEnvironment,
    commandOverrides: harmonyEnvironment ? {
      hdc: harmonyEnvironment.resolvedCommands?.hdc ?? undefined,
      hvigorw: harmonyEnvironment.resolvedCommands?.hvigor ?? undefined,
      "./hvigorw": harmonyEnvironment.resolvedCommands?.hvigor ?? undefined,
    } : {},
  });
  const request = {
