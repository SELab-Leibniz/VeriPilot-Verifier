...
export function parseKitManifest(markdown, {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if ((sectionBoundary || !inSection) && headingPattern.test(line)) {
      // A matching heading (re)opens a checklist section — a document may
      // carry several — and the first match names the sourceHint.
      inSection = true;
      if (sectionTitle === null) sectionTitle = line.replace(/^#{1,6}\s*/u, "") || null;
      continue;
    }
    if (sectionBoundary) {
      inSection = false;
    ...
    // A pipe row directly followed by a divider row is the table header —
    // its label cell (e.g. "Kit") would otherwise pass the shape filter.
