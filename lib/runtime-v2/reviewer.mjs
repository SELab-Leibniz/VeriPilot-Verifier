...
import { buildReviewerInvocation, decorateReviewerLaunchError, normalizeReviewerRuntime, resolveReviewerLaunchPlan } from "../reviewer-launcher.mjs";
...
function buildArguments({ prompt, sessionId, schema, reviewer, fork, pluginRoot = null }) {
...
async function execute({ cwd, env, args, launchPlan, timeoutMs, retryContext = null, deadlineAt = null }) {
...
export async function startRoleReviewer({
  ...
  try {
    ...
    first = await execute({
      ...
      args: buildArguments({
        prompt: rolePrompt,
        sessionId: freshSession ? null : continuationSessionId ?? parentSessionId,
        schema,
        reviewer,
        fork: !freshSession && !continuationSessionId,
        pluginRoot,
      }),
      timeoutMs: reviewer.timeoutMs,
