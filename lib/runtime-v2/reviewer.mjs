...
export function buildArguments({
  ...
  // A fresh Claude reviewer lets the CLI allocate its session ID. CodeAgent's
  // explicit dialect names every fresh session so concurrent reviewers never
  // depend on whichever session happens to be most recent.
  const args = [prompt, ...buildReviewerSessionArguments({
    sessionDialect,
    sessionId,
    newSessionId,
    fork,
...
function reconcileExpectedSession(parsed, expectedSessionId) {
  if (!expectedSessionId) return parsed;
  assertExpectedSession(parsed.sessionId, expectedSessionId);
  return { ...parsed, sessionId: parsed.sessionId ?? expectedSessionId };
}
...
async function execute({
  ...
  const invocation = buildReviewerInvocation(launchPlan, args);
  ...
  try {
    return {
      ...completed,
      ...reconcileExpectedSession(parseStructured(completed.stdout), expectedSessionId),
      executable,
      args,
    };
  } catch (error) {
    ...
    if (expectedSessionId) {
      assertExpectedSession(sessionId, expectedSessionId);
    ...
    const retryInvocation = buildReviewerInvocation(launchPlan, buildArguments({
      ...
      sessionDialect: launchPlan.sessionDialect,
    }));
    ...
    return {
      ...retried,
      ...reconcileExpectedSession(parseStructured(retried.stdout), expectedSessionId),
      executable,
      args,
    };
  }
...
async function repairStructuredResult({
  ...
  const repaired = await execute({
    ...
    deadlineAt,
    expectedSessionId: launchPlan.sessionDialect === "codeagent" ? first.sessionId : null,
  });
...
export async function startRoleReviewer({
  ...
  const launchPlan = resolvedLaunchPlan ?? await resolveReviewerLaunchPlan({ reviewerRuntime, env, projectRoot });
  ...
  // Both independent and detached start a FRESH session (no --resume).
  ...
  const state = { projectRoot, sessionCwd, taskId, parentSessionId, pluginRoot, role, reviewer, env, launchPlan, sessionPlan, deadlineAt, closed: false };
  ...
  try {
    ...
    const newSessionId = launchPlan.sessionDialect === "codeagent" && startsFreshSession
      ? randomUUID()
      : null;
    ...
    first = await execute({
      ...
      deadlineAt,
      expectedSessionId: launchPlan.sessionDialect === "codeagent" && !forkSession
        ? newSessionId ?? resumedSessionId
        : null,
    });
    if (launchPlan.sessionDialect === "codeagent" && forkSession && !first.sessionId) {
      throw new Error("CodeAgent forked reviewer did not return a session ID.");
    }
...
export async function handoffRoleReviewer({
  ...
  try {
    ...
    const launchPlan = await resolveReviewerLaunchPlan({ reviewerRuntime, env, projectRoot: options.projectRoot });
    ...
    const compatible = origin?.sessionId && origin.sessionPlan.session === "fork" && sessionPlan.session === "fork"
      ...
      && origin.launchPlan.sessionDialect === launchPlan.sessionDialect
      && ["projectRoot", "sessionCwd", "taskId", "parentSessionId", "pluginRoot"].every((key) => (origin[key] ?? null) === (options[key] ?? null))
      && [...PARENT_CREDENTIAL_VARIABLES, "CLAUDE_CONFIG_DIR"].every((key) => origin.env[key] === env[key]);
