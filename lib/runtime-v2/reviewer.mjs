...
import { buildReviewerInvocation, decorateReviewerLaunchError, resolveReviewerLaunchPlan } from "../reviewer-launcher.mjs";
...
export async function startRoleReviewer({
  ...
  const launchPlan = await resolveReviewerLaunchPlan({ reviewerRuntime, env, projectRoot });
  ...
  try {
    first = await execute({
      ...
      args: buildArguments({
        prompt: rolePrompt,
        sessionId: freshSession ? null : parentSessionId,
        schema,
        reviewer,
        fork: !freshSession,
        pluginRoot,
