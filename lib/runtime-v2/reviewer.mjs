...
async function execute({ cwd, env, args, timeoutMs, retryContext = null, deadlineAt = null }) {
  const executable = await resolveClaudeExecutable(env);
  const firstTimeoutMs = timeoutWithinDeadline(timeoutMs, deadlineAt);
  const completed = await spawnCaptured(executable, args, { cwd, env, timeoutMs: firstTimeoutMs });
  ...
  try {
    ...
    const retried = await spawnCaptured(executable, buildArguments({
      ...
      fork: false,
    }), { cwd, env, timeoutMs: retryTimeoutMs });
    if (retried.timedOut) {
...
async function repairStructuredResult({ first, schema, reviewer, sessionCwd, internalEnv, deadlineAt = null }) {
...
export async function startRoleReviewer({
  ...
  try {
    ...
    first = await repairStructuredResult({ first, schema, reviewer, sessionCwd, internalEnv, deadlineAt });
  } catch (error) {
