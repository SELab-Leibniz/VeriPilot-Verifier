...
const FINDING_SCHEMA = {
  ...
  properties: {
    ...
    targetFiles: { type: "array", items: { type: "string" }, maxItems: 100 },
  },
};
...
export const STOP_REVIEW_SCHEMA = {
  ...
  properties: {
    ...
    fileAssessments: { type: "array", maxItems: 200, items: {
      type: "object", additionalProperties: false, required: ["path", "status", "reason", "evidence"],
      properties: { path: { type: "string", minLength: 1 }, status: { type: "string", enum: ["PASS", "DEVIATION", "UNVERIFIED"] },
        reason: { type: "string", minLength: 1 }, evidence: { type: "array", items: { type: "string" }, minItems: 1 } },
    } },
...
export async function startRoleReviewer({
  ...
  env = sanitizeReviewerEnvironment(env);
...
export async function handoffRoleReviewer({
  ...
  try {
    ...
    const env = sanitizeReviewerEnvironment(options.env ?? origin?.env ?? process.env);
