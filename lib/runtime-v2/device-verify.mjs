...
export async function runCommand(command, args, { cwd, timeoutMs = DEFAULT_STEP_TIMEOUT_MS } = {}) {
  return new Promise((resolve) => {
    ...
    try {
      const child = execFile(command, args, { cwd, timeout: timeoutMs, maxBuffer: 4 * 1024 * 1024 }, (error, stdout, stderr) => {
        done({
          ok: !error,
 ...
 * pay the build twice.
 */
export async function runBuildGate({ projectRoot, adapter, budgetMs, execFn = runCommand, cacheFile = null, manifestDigest = null }) {
  ...
  const result = await execFn(build.command, build.args ?? [], { cwd: projectRoot, timeoutMs: budgetMs });
  // A build that fails only at packaging/signing did COMPILE. Charging that to
  // the developer would be the same error the whole ladder exists to avoid:
  // an environment that cannot complete a check must lower assurance, never
  // manufacture a defect.
  let environmentBlocked = false;
  if (!result.ok && build.environmentFailurePattern) {
    try {
      environmentBlocked = new RegExp(build.environmentFailurePattern, "iu")
        .test(`${result.stderr ?? ""}\n${result.stdout ?? ""}`);
    } catch {
      // An unusable pattern simply disables the classification.
    }
  }
  const outcome = {
    status: result.ok ? "passed" : (environmentBlocked ? "environment-blocked" : "failed"),
 ...
 */
export async function runDeviceSmoke({ projectRoot, adapter, outputDir, execFn = runCommand }) {
  ...
  for (const step of smoke.steps) {
    ...
    const result = await execFn(step.command, args, { cwd: projectRoot, timeoutMs: step.timeoutMs ?? DEFAULT_STEP_TIMEOUT_MS });
    steps.push({
...
export async function deviceVerification({
  ...
  const probe = await probeDeviceEnvironment({ projectRoot, adapter, execFn });
  ...
  if (requiredViolation) {
    findings.push({
      ...
      rootCauseId: "PROCESS_NOT_FOLLOWED",
      severity: "error",
  ...
  if (level === "build" || level === "device") {
    build = await runBuildGate({ projectRoot, adapter, budgetMs, execFn, cacheFile, manifestDigest });
  ...
  if (level === "device" && build.status === "passed") {
    smoke = await runDeviceSmoke({ projectRoot, adapter, outputDir, execFn });
