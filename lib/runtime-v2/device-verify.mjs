 ...
 */
async function evaluateProbe(probe, { projectRoot, execFn, commandOverrides = {} }) {
  ...
  const command = resolveCommand(probe.command, commandOverrides);
  const result = await execFn(command, probe.args ?? [], { cwd: projectRoot, timeoutMs: probe.timeoutMs ?? 10000 });
...
export async function probeDeviceEnvironment({ projectRoot, adapter, execFn = runCommand, commandOverrides = {} }) {
  ...
  return {
    declared: true,
    device: await evaluateProbe(deviceCheck.probes?.device, { projectRoot, execFn, commandOverrides }),
    toolchain: await evaluateProbe(deviceCheck.probes?.toolchain, { projectRoot, execFn, commandOverrides }),
  };
}


/**
 * Reuse task-scoped static toolchain facts while refreshing the volatile
 * connected-target state through the adapter declaration.
 */
export async function probeDeviceEnvironmentFromSnapshot(snapshot, adapter, {
  projectRoot,
  execFn = runCommand,
  commandOverrides = {},
  refreshTarget = true,
  ...
  const deviceCheck = adapter?.deviceCheck;
  const toolchainAvailable = snapshot.capabilities?.build?.state === "READY";
  const reusableTargetRefresh = refreshTarget && snapshot.targetRefresh?.attempted
    ? snapshot.targetRefresh
    : null;
  const device = deviceCheck && toolchainAvailable && refreshTarget
    ? reusableTargetRefresh?.device
      ?? await evaluateProbe(deviceCheck.probes?.device, { projectRoot, execFn, commandOverrides })
    : {
      available: false,
      detail: !refreshTarget
        ? "device mode off"
        : toolchainAvailable
          ? "platform declares no deviceCheck"
          : "cached toolchain is not ready",
    };
  return {
    declared: Boolean(deviceCheck),
    device,
    toolchain: {
    ...
    cached: true,
    targetRefreshed: Boolean(deviceCheck && toolchainAvailable && refreshTarget),
    targetRefreshReused: Boolean(reusableTargetRefresh),
    initialTargetState: reusableTargetRefresh?.initialState
      ?? snapshot.capabilities?.target?.state
      ?? "UNKNOWN",
...
export async function deviceVerification({
  ...
  const probe = environmentSnapshot
    ? await probeDeviceEnvironmentFromSnapshot(environmentSnapshot, adapter, {
      projectRoot,
      execFn,
      commandOverrides,
      refreshTarget: mode !== "off",
    })
    : await probeDeviceEnvironment({ projectRoot, adapter, execFn, commandOverrides });
