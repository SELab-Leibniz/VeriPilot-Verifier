...
function reviewerConfig(value, fallback) {
  return Object.freeze({
    ...
    timeoutMs: value?.timeoutMs ?? fallback.timeoutMs ?? activeHost.defaultReviewerTimeoutMs,
    maxBudgetUsd: value?.maxBudgetUsd ?? fallback.maxBudgetUsd ?? null,
    // fork reviews inside a --fork-session of the parent conversation;
    // detached starts a fresh session with ambient credentials; independent
    // spawns a FRESH session against provider (reviewer.mjs).
    session: value?.session ?? fallback.session ?? "fork",
    // Whether "fork" was CHOSEN or merely defaulted. Roles that need no
    // parent context (task onboarding) pick their own default, but must never
    // override a session the project configured explicitly.
    sessionExplicit: (value?.session ?? fallback.session) !== undefined,
...
export function compileRuntimeV2Config(config, { policyRoot = null, limits = {}, derived = null } = {}) {
  ...
  const loopInput = config.deviationLoop ?? {};
  ...
  const implementationCorrection = Object.freeze({
    ...
    // config/checklist-vocabulary.v1.json). Overridable so other document
    // conventions and languages need no code change.
    kitHeaderPattern: implementationInput.kitHeaderPattern ?? null,
    candidacyMarkers: implementationInput.candidacyMarkers ?? null,
    hedgeMarkers: implementationInput.hedgeMarkers ?? null,
  ...
  const reviewerDefaults = {
    effort: "low",
    timeoutMs: limits.semanticReviewTimeoutMs ?? activeHost.defaultReviewerTimeoutMs,
  ...
  return Object.freeze({
    ...
    // Per-family feedback loop bound. The corrector re-reviews after every
    // edit, so a finding the agent cannot satisfy is re-flagged indefinitely:
    // flag -> deliver -> edit -> re-flag. Observed in flower T1 2.5.0 where one
    // requirements family was re-flagged 7 times without closing. After
    // maxDeliveriesPerFamily DELIVERED rounds the family is parked: still
    // observed, still journaled, still counted in the V-metrics, but no longer
    // re-injected into the agent's context. 0 or null disables the bound.
    deviationLoop: Object.freeze({
      maxDeliveriesPerFamily: loopInput.maxDeliveriesPerFamily ?? 5,
    }),
