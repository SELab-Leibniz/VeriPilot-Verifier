...
const ONBOARDING_INSTRUCTIONS = [
  ...
  "For claims that require runtime verification, attach verification metadata. HarmonyOS page state, interaction, navigation, permission, accessibility, visual, persistence, or device behavior uses platform harmonyos, runtimeRequired true, modalities containing device/ui as applicable, and concrete evidenceKinds. Mark realDeviceOnly true only when the material explicitly excludes emulator verification. Pure source or manifest checks are not UI obligations.",
...
const ADJUDICATOR_INSTRUCTIONS = [
  ...
  "Preserve verification metadata for runtime/UI claims. Do not upgrade a source-only or ambiguous claim into a hard UI obligation, and preserve explicit realDeviceOnly limits.",
];
