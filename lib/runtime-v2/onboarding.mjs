...
// capabilityChecklist claims (audit step 9). After adjudication the entries
// are cross-checked deterministically against the platform adapter's catalog
// conventions; entries not resolvable to a platform module are kept but
// flagged catalogUnmatched (review-only — the deterministic kit checker in
// impl-review.mjs skips them, so they can never block a Stop).
//
...
import { kitModuleName } from "./impl-review.mjs";
...
const ONBOARDING_INSTRUCTIONS = [
  ...
  "Mine dependency/capability obligations into capabilityChecklist claims, each carrying capability.name (catalog-style lowercase-hyphenated capability or kit name), capability.module when the material states it, and capability.sourceHint (file or section). Use explicit dependency/kit tables when present (authority MATERIAL_DERIVED, severity HARD); otherwise infer from requirement semantics (authority AGENT_INFERRED, severity SOFT).",
...
const ADJUDICATOR_INSTRUCTIONS = [
  ...
  "disputedOperations were proposed by a minority or disagree between passes, and include open questions: decide each one SKEPTICALLY — prefer the narrower claim, prefer SOFT over HARD when the material evidence is thin, and drop claims without material support.",
...
export function panelClaimKey(operation) {
  ...
  if (category === "capabilityChecklist" && operation.capability?.name) {
    return `capabilityChecklist:${String(operation.capability.name).trim().toLowerCase()}`;
  }
...
export function crossCheckCapabilityOperations(operations, adapter) {
  ...
  const checked = operations.map((operation) => {
    ...
    const name = String(operation.capability.name).trim().toLowerCase();
 ...
 * task state. Every reviewer call respects the configured reviewer
 * timeoutMs/budget; passes run sequentially so peak spend stays bounded.
 */
export async function runTaskOnboarding({
  ...
  const passes = [];
  const failures = [];
  for (let passIndex = 1; passIndex <= panel.size; passIndex += 1) {
    let handle = null;
    try {
      ...
      passes.push(Array.isArray(handle.result?.operations) ? handle.result.operations : []);
    } catch (error) {
  ...
  const { majority, disputed } = mergePanelOperations(passes);
  let merged;
  ...
  if (panel.adjudicator) {
    ...
    try {
      handle = await reviewerFactory({
        ...
        reviewer: runtimeV2.reviewers.onboardingAdjudicator,
        schema: GROUND_TRUTH_REVIEW_SCHEMA,
        request: {
          ...
          panelProposals: passes,
          majorityOperations: majority,
          disputedOperations: disputed,
          materials,
      ...
      await journalDegraded(projectRoot, task.taskId, "ADJUDICATION_FAILED", { error: error.message });
      return { status: "DEGRADED", reason: "ADJUDICATION_FAILED", error: error.message };
    } finally {
      await handle?.close?.();
    }
  } else {
  ...
  const crossChecked = crossCheckCapabilityOperations(confirmed, adapter);
  let applied;
  try {
...
export async function maybeRunTaskOnboarding(context) {
  ...
  if (groundTruthVersion > 0 && !existing) {
    ...
    try {
      ...
      if (outcome?.status === "DEGRADED" && outcome.reason === "PANEL_FAILED" && (existing?.attempts ?? 0) < 2) {
        // Early-session fork failures are usually transient (parent session
        // not yet persisted); defer and let a later hook event retry.
        outcome = { status: "DEFERRED", reason: "PANEL_FAILED", attempts: (existing?.attempts ?? 0) + 1 };
      }
    } catch (error) {
