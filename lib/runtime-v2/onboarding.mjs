 ...
 */
export async function deterministicCapabilityOperations(materials, implementationCorrection = {}) {
  const operations = [];
  const seen = new Set();
  for (const entry of materials?.entries ?? []) {
    ...
    const { kits, sectionTitle } = parseKitManifest(content, {
    ...
    for (const kit of kits) {
      ...
      operations.push({
        ...
        text: `Capability checklist (${entry.path}${sectionTitle ? ` §${sectionTitle}` : ""}): integrate ${kit} in production source.`,
        authority: "MATERIAL_DERIVED",
        severity: "HARD",
        capability: { name: kit, sourceHint: `${entry.path}${sectionTitle ? `#${sectionTitle}` : ""}` },
      });
    }
  }
  return operations;
...
export async function runTaskOnboarding({
  ...
  const deterministicOps = await deterministicCapabilityOperations(materials, runtimeV2.implementationCorrection);
  ...
  const withDeterministic = confirmed.map((operation) => {
    ...
    return deterministicKeys.has(key)
