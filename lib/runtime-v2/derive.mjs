 ...
 * the compiler, relative for journals and the materialized config) plus the
 * platform fingerprint. The result feeds compileRuntimeV2Config as the
 * middle precedence tier — below explicit config, above plugin defaults.
 */
export async function deriveConfigDefaults(projectRoot) {
  const root = path.resolve(projectRoot);
