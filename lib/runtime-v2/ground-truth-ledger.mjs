...
function normalizeVerification(raw) {
  ...
  const allowedModalities = new Set(["build", "device", "ui"]);
  const allowedEvidenceKinds = new Set([
    "build", "install", "launch", "ui-action", "ui-assertion", "screenshot", "ui-dump", "restart",
  ]);
  return {
    platform: value.platform ? String(value.platform).trim().toLowerCase() : null,
    runtimeRequired: value.runtimeRequired === true,
    realDeviceOnly: value.realDeviceOnly === true,
    modalities: [...new Set((value.modalities ?? []).filter((item) => allowedModalities.has(item)))],
    evidenceKinds: [...new Set((value.evidenceKinds ?? []).filter((item) => allowedEvidenceKinds.has(item)))],
  };
}
...
function normalizeClaim(raw, operation, evidenceCapture, existing = null) {
  ...
  const verification = normalizeVerification(
    Object.hasOwn(raw, "verification") ? raw.verification : existing?.verification,
  );
  return {
    claimId,
    revision,
...
function renderGroundTruthMarkdown(document) {
  ...
  for (const category of categories) {
    ...
    for (const claim of document.claims.filter((item) => item.category === category)) {
      ...
      if (claim.verification?.runtimeRequired) {
        lines.push(`  - Verification: ${claim.verification.platform ?? "runtime"}/${claim.verification.modalities.join(",") || "runtime"}${claim.verification.realDeviceOnly ? " [real-device-only]" : ""}`);
      }
