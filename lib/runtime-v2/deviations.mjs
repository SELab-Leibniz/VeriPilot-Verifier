...
function normalizedFinding(finding) {
  return {
    ...
    ...(Array.isArray(finding.targetFiles) ? { targetFiles: finding.targetFiles.filter(file => typeof file === "string") } : {}),
  };
}
  ...
  await withTaskState({ projectRoot, taskId }, (state) => {
    ...
    for (const raw of findings) {
      ...
      state.deviations[familyId] = {
        ...
        firstFixedAt: previous?.firstFixedAt ?? previous?.fixedAt ?? null,
        fixedAt: previous?.status === "FIXED" ? null : previous?.fixedAt ?? null,
  ...
  const families = await withTaskState({ projectRoot, taskId }, (state) => {
    ...
    for (const deviation of Object.values(state.deviations)) {
      ...
      if (ids.length > 0 && ids.every((id) => passed.has(id))) {
        if (deviation.status === "DISMISSED") continue;
        const verifiedAt = new Date().toISOString();
        if (deviation.status !== "FIXED") deviation.fixedAt = verifiedAt;
        deviation.firstFixedAt ??= deviation.fixedAt;
        deviation.lastVerifiedAt = verifiedAt;
