...
export async function loadPlatformAdapter(platform = DEFAULT_PLATFORM) {
  ...
  return Object.freeze({
    ...
    kitCheck: Object.freeze({
      ...
      // Known capability names for this platform. Empty means "cannot
      // validate", which must never be treated as "everything is valid".
      catalog: new Set((kitCheck.catalog ?? []).map((name) => String(name).trim().toLowerCase())),
    }),
    ...
    // Where this platform keeps production sources, and which files belong in
    // the first-party review manifest. Data, not code: an unknown platform
    // simply falls back to the generic defaults in impl-review.
    sourceManifest: raw.sourceManifest ? Object.freeze(raw.sourceManifest) : null,
  });
}
