...
function findingIsBlocking(finding, hardIds) {
  ...
  // Deterministic kit-integration findings carry no ground-truth claim ids
  // (their authority is the §10.1 materials manifest, not a frozen claim), so
  // they block on their deviationKey namespace instead of the HARD-claim set.
  if (String(finding.deviationKey ?? "").startsWith("impl:kit:")) return true;
...
function stopFeedback(review, report, attempt, maximum, exhausted) {
  const lines = [
    `[runtime-corrector] Terminal correction ${attempt}/${maximum}`,
    review.summary,
  ];
  ...
  if (exhausted) lines.push("CORRECTION_BUDGET_EXHAUSTED: unresolved findings were recorded; this Stop is allowed.");
  else lines.push("Continue the task, correct these deviations, or respond with an evidence-based rejection.");
  return lines.join("\n");
}


async function assessStop({ input, projectRoot, sessionCwd, task, runtimeV2, reviewerFactory, groundTruthRefresh, snapshot }) {
  if (groundTruthRefresh.unverified) {
    const persisted = await persistEvaluation(projectRoot, task.taskId, "stop", {
      status: "UNVERIFIED",
      reason: "Ground Truth refresh failed; terminal correction failed open.",
      error: groundTruthRefresh.error?.message ?? null,
      transcriptDigest: snapshot.digest,
      transcriptCursor: snapshot.lastEntryKey,
    });
    // Fail-open must be loud: a silent UNVERIFIED here removed the terminal
    // detection source and skipped the final closure pass without anything
    // surfacing outside the evaluations directory.
    await appendTaskJournal(projectRoot, task.taskId, {
    ...
    return { decision: "allow", feedback: null, status: "UNVERIFIED", persisted };
  }
  ...
  try {
    ...
    if (hasBlocking && !exhausted) {
      return {
        ...
        correctionAttempt: attempt,
        correctionBudgetExhausted: false,
        feedback: stopFeedback(review, report, attempt, runtimeV2.stopCorrection.maxCorrectionsPerEpoch, false),
      };
    }
    return {
      decision: "allow",
    ...
    await appendTaskJournal(projectRoot, task.taskId, { type: "STOP_REVIEW_FAILED", error: error.message });
    await appendTaskJournal(projectRoot, task.taskId, { type: "STOP_ASSESSMENT_FAILED", reason: "STOP_REVIEW_EXCEPTION", error: error.message });
    return { decision: "allow", feedback: null, error, status: "UNVERIFIED" };
  } finally {
    if (handle) await handle.close();
  }
}
