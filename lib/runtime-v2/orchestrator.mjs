...
async function assessStop({ input, projectRoot, sessionCwd, task, runtimeV2, reviewerFactory, groundTruthRefresh, snapshot }) {
  if (groundTruthRefresh.unverified) {
    ...
    return {
      decision: "block",
      feedback: unverifiedStopFeedback(reason, groundTruthRefresh.error),
      status: "UNVERIFIED",
  ...
  try {
    ...
    const reason = "The final Stop reviewer failed before producing a valid assessment.";
    return {
      decision: "block",
      feedback: unverifiedStopFeedback(reason, error),
      error,
      status: "UNVERIFIED",
    };
  } finally {
