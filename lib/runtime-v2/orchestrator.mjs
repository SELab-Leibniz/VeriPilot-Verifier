...
const COMPLETION_CLAIM_PATTERN = /(?:(?:已|已经|现已|全部|均已|成功)(?:完成|实现|修复|修好|解决|交付|做好|做完)|(?:完成|实现|修复|修好|解决|交付|做好|做完)(?:了|完毕|并(?:验证|测试)?|[,，。！!]|$)|开发完毕|无需修改|无须修改|不需要修改|\bdone\b|\bcomplete(?:d)?\b|\bfinished\b|\bimplemented\b|\bfixed\b|\bresolved\b|\bdelivered\b|\bno\s+changes?\s+needed\b)/iu;
...
// FAIL CLOSED on a broken assessment: CHECKER_ERROR means the reviewer's
// judgements did not cover the frozen population (omitted, duplicated or
// unsupported labels), so "no blocking objects" is vacuous, not clean. The
// enterprise-collab T1 run 20260829-formal-001 stopped at the manual-tests
// stage with zero implementation through exactly this gap. The correction
// budget still bounds retries, so a persistently broken checker cannot trap
// the developer forever — it exhausts and releases with the record intact.
...
function stopFeedback(review, report, attempt, maximum, exhausted, locale = DEFAULT_LOCALE) {
  ...
  if (report.status === "CHECKER_ERROR") {
    const broken = report.metrics.flatMap((metric) =>
      metric.objects.filter((object) => object.judgement === "CHECKER_ERROR"));
    lines.push(formatMessage(locale, "stop.checkerError", { count: broken.length }));
    for (const object of broken.slice(0, 5)) lines.push(`- ${object.objectId}: ${object.reason}`);
  }
...
function claimsCompletion(input) {
  return COMPLETION_CLAIM_PATTERN.test(String(input.last_assistant_message ?? ""));
}
...
/**
 * SessionEnd is intentionally independent of project configuration. Ctrl+C
 * can interrupt config loading or onboarding, and teardown must never retry
 * either operation or hide Claude Code's native resume-session footer.
 */
export async function handleRuntimeV2SessionEnd({ input, projectRoot, env = process.env }) {
  try {
    ...
    // Teardown is always fail-open. In particular, do not route this failure
    // through the outer lifecycle warning recorder or retry onboarding.
...
async function handleRuntimeV2EventInternal({
  ...
  const shouldCrossBarrier = (
    ...
    input.hook_event_name === "PostToolUse" && task?.correctionBarrier?.turnActivated !== true
  ...
  // Prompt submission, compaction and batch notifications never mint a task.
