...
import { ensureHarmonyEnvironmentSnapshot, refreshHarmonyTarget } from "./harmony-environment.mjs";
  ...
  try {
    ...
    if (harmonyAssessment) {
      ...
      review.harmonyEnvironmentAssessment = {
        ...
        matchingFailureEvidence: harmonyAssessment.matchingFailureEvidence,
        obligations: harmonyAssessment.obligations,
        missingObligations: harmonyAssessment.missingObligations,
...
async function handleRuntimeV2EventInternal({
  ...
  if (input.hook_event_name === "Stop" && runtimeV2.stopCorrection.enabled) {
    const stopHarmonyEnvironment = harmonyEnvironment
      ? await refreshHarmonyTarget(harmonyEnvironment, {
        projectRoot,
        env,
        ...(harmonyEnvironmentExecFn ? { execFn: harmonyEnvironmentExecFn } : {}),
      })
    ...
    const stop = await assessStop({
      ...
      snapshot,
      harmonyEnvironment: stopHarmonyEnvironment,
    });
    return {
      handled: true,
      taskId: task.taskId,
