import { decisionScope } from "./decision-scope.mjs";
...
import { failedImplementationReview, mergeJudgementsByObjectId, runImplementationReview } from "./impl-review.mjs";
import { implementationPopulation, VERIFICATION_ROUTING_INSTRUCTIONS } from "./review-routing.mjs";
import { groundTruthSourceCatalog, rejectedGroundTruthSource, SOURCE_INSTRUCTIONS } from "./ground-truth-provenance.mjs";
...
import { processMetricJudgements } from "./process-checks.mjs";
import {
...
async function refreshGroundTruth({
  ...
  const sourceCatalog = await groundTruthSourceCatalog({ snapshot, materials, projectRoot, skill });
  ...
  const request = {
    ...
    instructions: [
      ...VERIFICATION_ROUTING_INSTRUCTIONS,
      ...SOURCE_INSTRUCTIONS,
  ...
  try {
    handle = await reviewerFactory({
      ...
      request,
      evidence: { snapshot, groundTruth: current },
    });
    ...
    if (applied.droppedUntrustedSources?.length && applied.current.claims.length === 0) {
      throw new Error("No valid task Ground Truth remains after source validation; rejected internal/unknown sources cannot establish an empty PASS baseline.");
    }
    ...
    if (current.version > 0 && applied.hardChanged && delta.taskClassification !== "NEW_TASK"
      && decisionScope.getStore()?.context?.mode !== "verify_only") {
...
async function assessWatcher({
  ...
  try {
    ...
    const originHandle = handle;
    handle = null; // handoff consumes the source, including on failure.
    handle = await reviewerFactory.handoff({
      originHandle,
      projectRoot,
      sessionCwd,
      taskId: task.taskId,
      ...
      request,
      evidence: { snapshot, groundTruth: groundTruthRefresh?.current, skillGroundTruth },
    });
    const review = handle.result;
...
function hardClaimIds(groundTruth) {
  return new Set((groundTruth.claims ?? []).filter((claim) => (
    claim.status === "ACTIVE" && claim.severity === "HARD" && !rejectedGroundTruthSource(claim)
...
function passedMetricSourceIds(report) {
  const objects = report.metrics.flatMap((metric) => metric.objects);
  const unresolved = new Set(objects.filter((object) => object.judgement !== "PASS")
    .map((object) => object.sourceId));
  return objects
    .filter((object) => object.judgement === "PASS" && !unresolved.has(object.sourceId))
    .map((object) => object.sourceId);
}
...
function stopFeedback(review, report, attempt, maximum, exhausted, locale = DEFAULT_LOCALE, released = exhausted) {
  const lines = [
  ...
  const checkerErrorDetails = (report.metrics ?? [])
    .flatMap((metric) => metric.objects)
    .filter((object) => object.judgement === "CHECKER_ERROR")
    .map((object) => ({ objectId: object.objectId, reason: object.reason }));
  checkerErrorDetails.push(...(report.checkerIssues ?? []).map((issue) => ({
    objectId: issue.objectId ?? issue.type ?? "unknown",
    reason: issue.message ?? issue.type ?? "Unknown checker issue.",
  })));
  if (report.status === "CHECKER_ERROR") {
    lines.push(formatMessage(locale, "stop.checkerErrorSummary", {
      count: checkerErrorDetails.length,
    }));
  }
  for (const object of checkerErrorDetails.slice(0, 5)) {
  ...
  if (released) {
    const unresolved = (report.metrics ?? []).flatMap((metric) => metric.objects)
      .filter((object) => ["UNVERIFIED", "EXTERNAL_BLOCKED"].includes(object.judgement));
    for (const object of unresolved.slice(0, 5)) lines.push(`- ${object.objectId}: ${object.reason}`);
  }
  lines.push(formatMessage(locale, exhausted ? "stop.exhausted" : released ? "stop.incomplete" : "stop.continue"));
  return lines.join("\n");
}
...
async function stopInfrastructureFailure({ projectRoot, taskId, reason, error = null, persisted = null, locale = DEFAULT_LOCALE }) {
  ...
  await withTaskState({ projectRoot, taskId }, (state) => {
    ...
    state.stop.infrastructureTotalFailures = (state.stop.infrastructureTotalFailures ?? 0) + 1;
    attempt = state.stop.infrastructureFailures;
    state.verification = {
      status: "UNVERIFIED", reason, evaluationId: persisted?.evaluationId ?? null,
      updatedAt: new Date().toISOString(),
    };
    if (persisted?.evaluationId) state.stop.lastAssessmentId = persisted.evaluationId;
    if (attempt > MAX_STOP_INFRASTRUCTURE_RETRIES) state.status = "STOPPED_UNVERIFIED";
  });
...
async function assessStop({ input, projectRoot, sessionCwd, task, runtimeV2, reviewerFactory, groundTruthRefresh, snapshot, deviceVerifier = null }) {
  const verifyOnly = decisionScope.getStore()?.context?.mode === "verify_only";
  if (groundTruthRefresh.unverified) {
  ...
  try {
    ...
    const request = {
      ...
      assessmentMode: verifyOnly ? "verify_only" : "execute_and_correct",
      instructions: [
        ...(verifyOnly ? ["This is an explicit user-requested, read-only verification of the CURRENT output, not a worker asking permission to start. Evaluate the required output now as TASK_COMPLETE (or STAGE_COMPLETE for a scoped stage). Completion classification is an assessment scope, not a claim that files pass. Waiting for authorization to MODIFY files does not prevent this verification. Report actual missing or contradictory requirements as unresolved evidence; never invent them."] : []),
        ...
        "For file-specific findings, set targetFiles to the affected file paths. Cite upstream reference files in actualEvidence, not as defective targets. Do not invent a path for a task-wide finding.",
        "For each requested deliverable file, also return fileAssessments describing its CURRENT content quality with path, status, reason and read evidence. Judge against the intended artifact requirements, separately from whether a fault-injection test successfully produced or detected a defect. Successfully detecting an intentionally broken design/tasks file MUST NOT mark that file PASS. Use UNVERIFIED when the applicable quality baseline or file evidence is insufficient. These supplemental file verdicts do not replace the frozen metric judgements.",
        `Use only these rootCauseId values: ${[...ROOT_CAUSE_IDS].join(", ")}.`,
      ],
      taskId: task.taskId,
      hook: sanitizedHookInput(input),
    ...
    handle = await reviewerFactory.handoff({
      originHandle,
      closeOrigin: (source) => closeStopReviewerSafely(source, projectRoot, task.taskId),
      projectRoot,
      sessionCwd,
      taskId: task.taskId,
      parentSessionId: input.session_id,
      ...
      request,
      evidence: { snapshot, groundTruth: current, population },
    });
    review = handle.result;
    review.providerDegradation = handle?.providerDegradation ?? null;
    ...
    if (!new Set(["STAGE_COMPLETE", "TASK_COMPLETE"]).has(review.stopClassification)) {
      ...
      if (hasBlocking) {
        await recordDeviationFindings({
          ...
          delivered: !verifyOnly && runtimeV2.shadowMode !== true && !exhausted,
        });
      }
      await withTaskState({ projectRoot, taskId: task.taskId }, (state) => {
        ...
        state.verification = {
          status: hasBlocking ? "DEVIATION" : "NOT_YET_EXECUTED",
          evaluationId: persisted.evaluationId, correctionBudgetExhausted: exhausted,
          updatedAt: new Date().toISOString(),
        };
        if (hasBlocking && exhausted) state.status = "STOPPED_UNVERIFIED";
        else if (hasBlocking && verifyOnly) state.status = "AWAITING_CORRECTION";
      });
      ...
      return {
        ...
        correctionAttempt: attempt,
        correctionBudgetExhausted: exhausted,
        verificationIncomplete: hasBlocking && exhausted,
        feedback: hasBlocking && exhausted ? stopFeedback(review, { blockingObjects: [] }, attempt,
          runtimeV2.stopCorrection.maxCorrectionsPerEpoch, true, runtimeV2.locale) : null,
      };
    }
    ...
    // path. Faults use the bounded infrastructure retry path, not PASS fallback.
    let implementationReview = null;
    const stopReviewerSummary = review.summary;
    const stopReviewerJudgements = [...review.metricObjectJudgements];
    if (runtimeV2.implementationCorrection?.enabled) {
      try {
        const implReview = await runImplementationReview({
          ...
          stage: review.stopClassification === "STAGE_COMPLETE" ? review.stage : null,
        });
        ...
        await appendTaskJournal(projectRoot, task.taskId, {
          type: `IMPLEMENTATION_REVIEW_${implReview.status ?? "COMPLETED"}`,
          skipReason: implReview.skipReason ?? null,
          error: implReview.reviewerError ?? null,
        ...
        implementationReview = failedImplementationReview(
          implementationPopulation(population, { groundTruth: current,
            stage: review.stopClassification === "STAGE_COMPLETE" ? review.stage : null }).population, error,
        );
        review.metricObjectJudgements = mergeJudgementsByObjectId(
          review.metricObjectJudgements, implementationReview.metricObjectJudgements,
        );
        await appendTaskJournal(projectRoot, task.taskId, {
    ...
    const processJudgements = processMetricJudgements({ population, snapshot });
    review.metricObjectJudgements = mergeJudgementsByObjectId(review.metricObjectJudgements, processJudgements);
    const report = calculateMetricReport({
      population,
      judgements: review.metricObjectJudgements,
    });
    const implementationFailed = implementationReview?.status === "FAILED";
    if (implementationFailed) {
      report.checkerIssues.push({ type: "IMPLEMENTATION_REVIEW_FAILED", message: implementationReview.reviewerError });
      report.status = "CHECKER_ERROR";
    }
    const objects = report.metrics.flatMap((metric) => metric.objects);
    review.stopReviewerSummary = stopReviewerSummary;
    review.stopReviewerJudgements = stopReviewerJudgements;
    review.summary = formatMessage(runtimeV2.locale, "stop.assessmentSummary", {
      status: blockingFindings.length && report.status === "PASS" ? "DEVIATION" : report.status,
      passed: objects.filter((object) => object.judgement === "PASS").length,
      total: objects.length, blocking: report.blockingObjects.length + blockingFindings.length,
    });
    const implIds = new Set((implementationReview?.metricObjectJudgements ?? []).map((object) => object.objectId));
    const processIds = new Set(processJudgements.map((object) => object.objectId));
    for (const object of objects) object.reviewer = processIds.has(object.objectId)
      ? "process-checker" : implIds.has(object.objectId) ? "implementation-reviewer" : "stop-reviewer";
    const hasBlocking = stopAssessmentBlocks(report, blockingFindings);
    const checkerFailed = report.status === "CHECKER_ERROR";
    let attempt = 0;
    let exhausted = false;
    await withTaskState({ projectRoot, taskId: task.taskId }, (state) => {
      if (!verifyOnly && state.stop.epochId !== state.correctionEpoch.id) {
      ...
      if (hasBlocking && !checkerFailed && !verifyOnly) {
        if (state.stop.correctionAttempts < runtimeV2.stopCorrection.maxCorrectionsPerEpoch) {
          state.stop.correctionAttempts += 1;
        } else {
          exhausted = true;
    ...
    if (checkerFailed) {
      const failure = await stopInfrastructureFailure({
        projectRoot, taskId: task.taskId, persisted, locale: runtimeV2.locale,
        reason: implementationFailed
          ? "Required implementation verification failed; general reviewer PASS is not a substitute."
          : "The reviewer did not return a valid assessment of the frozen population.",
        error: new Error(implementationFailed ? implementationReview.reviewerError : [
          ...report.checkerIssues.map((issue) => issue.message),
          ...objects.filter((object) => object.judgement === "CHECKER_ERROR").map((object) => `${object.objectId}: ${object.reason}`),
        ].slice(0, 5).join("; ")),
      });
      return { ...failure, review, report, implementationReview };
    }
    ...
    await recordDeviationFindings({
      ...
      delivered: !verifyOnly && runtimeV2.shadowMode !== true && hasBlocking && !exhausted,
    });
    await withTaskState({ projectRoot, taskId: task.taskId }, (state) => {
      state.stop.lastAssessmentId = persisted.evaluationId;
      state.stop.infrastructureFailures = 0;
      ...
      state.verification = {
        status: hasBlocking && report.status === "PASS" ? "DEVIATION" : report.status,
        evaluationId: persisted.evaluationId, correctionBudgetExhausted: exhausted,
        updatedAt: new Date().toISOString(),
      };
      if ((!hasBlocking || exhausted) && review.stopClassification === "TASK_COMPLETE") {
        state.status = !hasBlocking && report.status === "PASS" ? "COMPLETED" : "COMPLETED_WITH_ISSUES";
      }
      if (hasBlocking && !exhausted && verifyOnly) state.status = "AWAITING_CORRECTION";
    });
    if (hasBlocking && !exhausted) {
      return {
        decision: "block",
    ...
    return {
      ...
      correctionAttempt: attempt,
      correctionBudgetExhausted: exhausted,
      verificationIncomplete: hasBlocking || report.status !== "PASS",
      feedback: hasBlocking || report.status !== "PASS"
        ? stopFeedback(review, report, attempt, runtimeV2.stopCorrection.maxCorrectionsPerEpoch, exhausted, runtimeV2.locale, true)
        : null,
    };
  } catch (error) {
...
function claimsCompletion(input) {
  ...
  if (INCOMPLETE_CLAIM_PATTERN.test(message) || RESIDUAL_COMPLETION_WORK_PATTERN.test(message)) return false;
...
async function crossCorrectionBarrier({
  ...
  await withTaskState({ projectRoot, taskId: activeTask.taskId }, (state) => {
    const alreadyActivated = state.correctionBarrier?.turnActivated === true;
    state.correctionBarrier = {
      ...(state.correctionBarrier ?? {}),
      turnActivated: alreadyActivated,
      lastHookEventId: eventId(input),
      lastTriggerEvent: input.hook_event_name,
      lastToolName: input.tool_name ?? null,
      activatedAt: state.correctionBarrier?.activatedAt ?? null,
    };
...
export async function handleRuntimeV2Event(options) {
  const mode = options.assessmentContext?.mode ?? "execute_and_correct";
  if (!["execute_and_correct", "verify_only"].includes(mode)) throw new Error("Invalid assessment mode.");
  let result;
  if (options.assessmentContext && options.input.hook_event_name === "Stop") {
    let task = await findTask({ projectRoot: options.projectRoot, sessionId: options.input.session_id });
    if (!task && claimsCompletion(options.input)) {
      task = await ensureTask({ projectRoot: options.projectRoot, sessionId: options.input.session_id });
      await options.assessmentContext.onTaskCreated?.(task);
      task = await findTask({ projectRoot: options.projectRoot, sessionId: options.input.session_id });
      options = { ...options, assessmentContext: { ...options.assessmentContext, guard: decisionGuard(task) } };
    }
    result = task ? await withTaskDecision({ projectRoot: options.projectRoot, taskId: task.taskId,
      context: { ...options.assessmentContext, mode } }, () => handleRuntimeV2EventInternal(options))
      : await handleRuntimeV2EventInternal(options);
    result = { ...result, assessmentMode: mode,
      ...(result.stop ? { stop: { ...result.stop, assessmentMode: mode } } : {}) };
  } else result = await handleRuntimeV2EventInternal(options);
...
export async function refreshRuntimeRequirements({ projectRoot, input, plan, reviewerFactory, assessmentContext }) {
  const task = await findTask({ projectRoot, sessionId: input.session_id });
  if (!task) throw new Error("No existing task for requirement update.");
  return withTaskDecision({ projectRoot, taskId: task.taskId, context: { ...assessmentContext, kind: "requirements" } }, async () => {
    const snapshot = await readTranscriptSnapshot(input.transcript_path);
    const refresh = await refreshGroundTruth({ input: { ...input, hook_event_name: "Stop" }, projectRoot, sessionCwd: input.cwd, task,
      runtimeV2: plan.runtimeV2, reviewerFactory, snapshot });
    await refresh.handle?.close();
    if (refresh.unverified) throw new Error("Requirement baseline extraction failed; requirement remains pending.");
    await withTaskState({ projectRoot, taskId: task.taskId }, (state) => {
      state.control.pendingRequirement = null;
      state.status = "ACTIVE";
      state.verification = { ...state.verification, status: "STALE", reason: "REQUIREMENTS_UPDATED" };
      state.pendingCorrection = null;
    });
    return { handled: true, taskId: task.taskId, requirementVersion: refresh.current.version, baselineCommitted: true };
  });
}
...
async function handleRuntimeV2EventInternal({
  ...
  const canonicalReviewerFactory = (options) => reviewerFactory({
    reviewerRuntime: plan.reviewerRuntime,
    ...options,
    env: options.env ?? { ...env },
    pluginRoot: options.pluginRoot ?? pluginRoot,
  });
  // Keep ambient execution state in this closure, never in persisted evidence.
  canonicalReviewerFactory.handoff = (options) => (reviewerFactory.handoff ?? handoffRoleReviewer)({
    reviewerRuntime: plan.reviewerRuntime,
    ...options,
    env: options.env ?? { ...env },
    pluginRoot: options.pluginRoot ?? pluginRoot,
    reviewerFactory: canonicalReviewerFactory,
  });
  ...
  if (input.hook_event_name === "Stop") {
    if (!String(input.last_assistant_message ?? "").trim()) {
      ...
      const transcriptMessage = latestAssistantText(snapshot);
      if (transcriptMessage) input = { ...input, last_assistant_message: transcriptMessage };
    }
    const shouldRunFullStop = decisionScope.getStore()?.context?.mode === "verify_only"
  ...
  const watcherBatchIncomplete = input.hook_event_name === "PostToolUse"
    && unresolvedSiblingToolUseIds(snapshot, input.tool_use_id).length > 0;
  if (SKILL_CHECK_EVENTS.has(input.hook_event_name)
    && runtimeV2.skillCorrection.enabled
    && !watcherBatchIncomplete) {
  ...
  if (input.hook_event_name === "Stop"
    ...
    const watcherFeedback = feedback.join("\n\n");
    const stopFeedbackForAgent = watcherFeedback && stop.verificationUnavailable
      ? null
      : stop.feedback;
    return {
      handled: true,
      ...
      feedback: [watcherFeedback, stopFeedbackForAgent].filter(Boolean).join("\n\n") || null,
      stop,
    };
  }
  ...
  if (artifact && input.hook_event_name === "PostToolUse") {
    ...
    if (artifact.metricCheckpoint && runtimeV2.artifactCorrection.stageMetricsEnabled) {
      try {
        ...
        // Ownership has not yet passed to semantic review. Preserve the
        // preparation error while attempting to release the source resources.
        await Promise.allSettled([Promise.resolve().then(() => refresh.handle?.close())]);
        throw error;
      }
    }
    return {
      ...
      reviewerEvidence: { snapshot, groundTruth: refresh.current, population },
      // Execution-only state must never enter the serialized artifact evidence.
