...
function stopFeedback(review, report, attempt, maximum, exhausted) {
  ...
  if (assurance) {
    ...
    lines.push(assurance.level === "device"
      ? `Assurance: device-level verification ran (build: ${assurance.build}, smoke: ${assurance.smoke}).`
      : `Assurance: ${assurance.level}-level verification only (${assurance.reason}); device-dependent checks were skipped, not judged.`);
  }
