...
async function assessStop({ input, projectRoot, sessionCwd, task, runtimeV2, reviewerFactory, groundTruthRefresh, snapshot, deviceVerifier = null }) {
  ...
  let review;
  try {
    // A cross-role follow-up inherits the old process environment. It cannot
    // implement an explicitly independent Stop provider by changing model alone.
    if (handle && runtimeV2.reviewers.stopReviewer.session === "independent") {
