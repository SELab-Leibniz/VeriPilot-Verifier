...
const EVENT_REQUIRED_FIELDS = Object.freeze({
  ...
  PostToolUse: { tool_name: STRING, tool_input: OBJECT, tool_response: PRESENT, tool_use_id: STRING },
  Stop: { stop_hook_active: BOOLEAN },
  PreCompact: { trigger: STRING, custom_instructions: STRING_OR_NULL },
...
export function decodeHookInput(raw) {
  ...
  if (!OBJECT(input)) throw new TypeError("Hook input must be a JSON object");
  ...
  assertRequiredFields(input, EVENT_REQUIRED_FIELDS[input.hook_event_name], input.hook_event_name);
  return { ...input };
}

function contextOutput(eventName, feedback) {
  if (!STRING(feedback) || feedback.length === 0) return null;
  return { hookSpecificOutput: { hookEventName: eventName, additionalContext: feedback } };
}

export function encodeHookOutput(eventName, input, outcome) {
  if (eventName === "Stop") {
    if (outcome?.decision === "block") {
      return STRING(outcome.feedback) ? { decision: "block", reason: outcome.feedback } : null;
    }
    if (outcome?.verificationUnavailable || outcome?.stop?.verificationUnavailable
      || outcome?.verificationIncomplete || outcome?.stop?.verificationIncomplete) {
      return STRING(outcome.feedback) ? { continue: true, systemMessage: outcome.feedback } : null;
    }
    return null;
  }
