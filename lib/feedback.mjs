...
// Cap inline lists only when their full persisted counterparts exist.
...
export function formatAgentFeedback(
  ...
  if (result.diagnostics.length === 0) {
    ...
    const shown = diagnosticOutput ? ranked.slice(0, MAX_INLINE_DIAGNOSTICS) : ranked;
    lines.push(
  ...
  const inlineDiffs = diffOutput ? result.diffs.slice(0, MAX_INLINE_DIFFS) : result.diffs;
