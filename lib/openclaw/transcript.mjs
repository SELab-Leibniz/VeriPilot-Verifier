...
import { projectTrustedActions } from "./interaction-router.mjs";
...
export function normalizeMessages(records = []) {
  ...
  return records.flatMap((record, index) => {
    ...
    return [{ type: role === "assistant" ? "assistant" : "user", uuid: id,
      ...(synthetic ? { isMeta: true } : {}), sourceMessageId: message.idempotencyKey ?? message.id ?? null, timestamp: record.timestamp ?? message.timestamp,
...
export async function persistTranscript(state, { messages, transcriptPath, prompt, realUser = false, runId } = {}) {
  ...
  if (!state.sourceMappings) {
    try { state.sourceMappings = JSON.parse(await fs.readFile(path.join(path.dirname(state.transcriptPath), "source-mappings.json"), "utf8")); }
    catch (error) { if (error.code !== "ENOENT") throw error; state.sourceMappings = {}; }
  }
  const actionDirectory = path.join(path.dirname(state.transcriptPath), "source-actions");
  let actionFiles = [];
  try { actionFiles = await fs.readdir(actionDirectory); } catch (error) { if (error.code !== "ENOENT") throw error; }
  for (const file of actionFiles.filter(name => name.endsWith(".json"))) {
    const mapping = JSON.parse(await fs.readFile(path.join(actionDirectory, file), "utf8"));
    const previous = state.sourceMappings[mapping.messageId];
    if (previous && JSON.stringify(previous.actions) !== JSON.stringify(mapping.actions)) throw new Error("Trusted message action mapping conflict.");
    state.sourceMappings[mapping.messageId] = mapping;
  }
  let entries;
  ...
  if (entries) {
    ...
    for (const entry of entries.filter((item) => item.type === "user" && !item.isMeta)) {
      const index = unmatched.findIndex((prior) => prior.nativeUuid === entry.uuid || prior.uuid === entry.uuid
        || (entry.sourceMessageId && prior.sourceMessageId === entry.sourceMessageId));
      if (index < 0) continue;
    ...
    state.entries = projectTrustedActions(entries, state.sourceMappings);
  }
  state.entries ??= [];
  if (typeof prompt === "string" && prompt.trim() && realUser) {
    ...
    if (!state.entries.some((entry) => entry.uuid === id)) {
      state.entries.push({ type: "user", uuid: id, pendingPrompt: true, sourceMessageId: runId, message: { id, content } });
    }
  }
  // Write atomically so parallel hooks never see a half-written snapshot.
  state.entries = projectTrustedActions(state.entries, state.sourceMappings);
...
export async function recordSourceActions(binding, messageId, actions) {
  const directory = path.join(binding.workspaceDir, ".runtime-correction", "openclaw",
    digest([binding.agentId, binding.sessionId]).slice(0, 24), "source-actions");
  const file = path.join(directory, `${digest(messageId)}.json`);
  return withTaskResourceLock({ projectRoot: binding.workspaceDir, taskId: `source-${digest(binding.sessionId)}`,
    resource: digest(messageId), timeoutMs: 1000, reclaimDeadOwner: true }, async () => {
    let previous;
    try { previous = JSON.parse(await fs.readFile(file, "utf8")); } catch (error) { if (error.code !== "ENOENT") throw error; }
    if (previous && JSON.stringify(previous.actions) !== JSON.stringify(actions)) throw new Error("Trusted message action mapping conflict.");
    if (!previous) await privateJson(file, { messageId, actions });
  });
}

export async function saveSourceMapping(state, messageId, actions, aliases = []) {
  await persistTranscript(state);
  const prior = state.sourceMappings[messageId];
  if (prior && JSON.stringify(prior.actions) !== JSON.stringify(actions)) throw new Error("Trusted message action mapping conflict.");
  const mapping = prior ?? { messageId, actions };
  for (const id of [messageId, ...aliases].filter(Boolean)) {
    if (state.sourceMappings[id] && state.sourceMappings[id].messageId !== messageId) throw new Error("Native identity mapping conflict.");
    state.sourceMappings[id] = mapping;
  }
  await privateJson(path.join(path.dirname(state.transcriptPath), "source-mappings.json"), state.sourceMappings);
}
