...
export function createOpenClawRuntime(api, { pluginRoot, reviewerFactory: injectedFactory, sharedState = {} } = {}) {
  ...
  function internal(event, ctx) {
    ...
    return internalSessions.has(id) || String(ctx.sessionKey ?? event.sessionKey ?? "").includes(INTERNAL_SESSION_MARKER);
  }
  ...
  const handlers = {
    ...
    async beforeTool(event, ctx) {
      ...
      if (callId && ctx.sessionId) {
        ...
        bindings.set(`${ctx.agentId ?? "main"}:${ctx.sessionId}`, { ...ctx });
    ...
    async reply(event) {
      if (settings.enabled === false || event.kind !== "final" || !event.runId) return;
