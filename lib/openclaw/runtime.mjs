...
import { findTask } from "../runtime-v2/task-store.mjs";
...
export function createOpenClawRuntime(api, { pluginRoot, reviewerFactory: injectedFactory, sharedState = {} } = {}) {
  ...
  async function context(state, hookName) {
    ...
    const reviewerFactory = injectedFactory ?? createOpenClawReviewerFactory(api, {
      ...
      deadlineAt: Math.min(Date.now() + hookBudget(hookName), state.runDeadlineAt ?? Infinity), internalSessions,
      ...
      return withState(event, ctx, async (state) => {
        ...
        try {
          ...
          if (state.observedPaths) {
            ...
            if (String(event.toolName).toLowerCase() === "read") candidates.push(args.path ?? args.file_path);
  ...
  return { ...handlers, internalSessions,
    ...
    async beginSupervised(params, abortSignal, { deadlineAt } = {}) {
      ...
      await handlers.prompt({ prompt: params.transcriptPrompt ?? params.prompt }, { ...ctx, trigger: "user" });
      return state;
    },
    ...
    async assessSupervised(state, { transcriptPath, text, assessmentId, stopHookActive, abortSignal = state.abortSignal }) {
      return queue(state, async () => {
        ...
        const value = await context(state, "before_agent_finalize");
