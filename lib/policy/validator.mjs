...
export async function validateProjectPolicy({ cwd, pluginRoot, env = process.env } = {}) {
  ...
  const issues = [];
  for (const [role, reviewer] of Object.entries(plan.runtimeV2?.reviewers ?? {})) {
    if (reviewer.session !== "independent") continue;
    const name = reviewer.provider?.apiKeyEnv;
    if (!name || !env[name]?.trim()) issues.push(validationIssue(
      "warning", "REVIEWER-PROVIDER-UNAVAILABLE",
      `独立审阅器 ${role} 在当前校验进程中找不到密钥变量 ${name ?? "(未配置)"}；运行时将回退，不能声称使用了独立提供方。`,
      `reviewer:${role}`,
    ));
  }
