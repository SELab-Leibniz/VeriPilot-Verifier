...
import { normalizeReviewerRuntime } from "../reviewer-launcher.mjs";
import { host } from "../active-host.mjs";
...
export function compileRuntimePolicy(
  ...
  if (host === "openclaw" && config.reviewerRuntime !== undefined) {
    throw Object.assign(new Error(`${context.configPath ?? ".runtime-corrector/config.yaml"}: OpenClaw does not use reviewerRuntime. Remove the entire reviewerRuntime block (including executable and argsPrefix); select reviewers with plugins.entries.runtime-corrector.config.reviewerModel or reviewers.<role>.model.`),
      { code: "OPENCLAW_REVIEWER_CONFIG" });
  }
  const reviewerRuntime = normalizeReviewerRuntime(config.reviewerRuntime, {
    projectRoot: typeof context.policyRoot === "string" && path.isAbsolute(context.policyRoot)
      ? path.dirname(context.policyRoot)
      : projectRoot,
  });
  ...
  return {
    ...publicConfig,
    ...(reviewerRuntime === undefined ? {} : { reviewerRuntime }),
    artifacts,
    configuredArtifacts,
    groundTruthSources,
    reviewGraph,
