...
1. **Panel decomposition.** `dynamicGroundTruth.panel.size` (default 2) independent `onboarding-extractor` passes — the standard reviewer spawn machinery and extractor schema, one fork each — decompose ALL task materials into atomic claims in a single dedicated pass per reviewer. Panel passes reuse the `groundTruthExtractor` reviewer configuration (model/effort/timeoutMs/budget) and run sequentially.
